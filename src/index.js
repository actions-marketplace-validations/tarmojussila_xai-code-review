...
const COMMENT_MARKER = '<!-- xai-code-review -->';
...
function callXaiApi(apiKey, model, systemPrompt, prompt) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request(options, res => {
      ...
      res.on('data', chunk => {
        ...
        if (data.length > MAX_RESPONSE_SIZE) {
          req.destroy(new Error('xAI API response exceeded size limit.'));
        }
      });
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          try {
            ...
            reject(new Error('xAI API returned invalid JSON.'));
            return;
          }
          const content = parsed.choices?.[0]?.message?.content;
          if (!content) {
            reject(new Error('xAI API returned an empty response.'));
          } else {
            resolve(content);
          }
        } else {
          reject(new Error(`xAI API error ${res.statusCode}: ${data.slice(0, 200)}`));
        }
      });
    });
    ...
    req.setTimeout(REQUEST_TIMEOUT_MS, () => {
      req.destroy(new Error('xAI API request timed out.'));
    });
    req.write(body);
    req.end();
  });
...
async function run() {
  const apiKey = core.getInput('XAI_API_KEY', { required: true });
  core.setSecret(apiKey);
  const model = core.getInput('XAI_MODEL');
  const systemPrompt = core.getInput('XAI_SYSTEM_PROMPT');
  const reviewerName = core.getInput('XAI_REVIEWER_NAME');
  ...
  core.info(`Sending ${filteredFiles.length} file(s) to xAI for review...`);
  const review = await callXaiApi(apiKey, model, systemPrompt, prompt);
